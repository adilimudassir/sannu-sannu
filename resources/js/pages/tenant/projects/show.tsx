...
export default function TenantProjectShow({ 
  project, 
  statistics, 
  canEdit, 
  canDelete, 
  ...
  const getBackUrl = () => {
    return route('tenant.projects.index', project.tenant?.slug);
  };

  const getEditUrl = () => {
    return route('tenant.projects.edit', [project.tenant?.slug, project.id]);
  };

  const handleDelete = () => {
    const deleteRoute = route('tenant.projects.destroy', [project.tenant?.slug, project.id]);
  ...
  const handleStatusChange = (action: 'activate' | 'pause' | 'resume' | 'complete' | 'cancel') => {
    const actionRoute = route(`tenant.projects.${action}`, [project.tenant?.slug, project.id]);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                ...
                {project.tenant && ` • ${project.tenant.name}`}
              </p>
            </div>
          </div>
